---
layout: default
title: Etsy
permalink: /etsy/
menu: false
---
<div class="etsy">
	<h4 class='job-header'>Full Stack Developer Job Application for Etsy </h4>
	<div class='application-layout-content'>
		<p>Hi my name is Niamh and I would like to apply for the position of Full Stack Developer at Etsy.</p>
		<p> I appreciate your consideration of my application. I elected to put this page together as a way displaying my personality, experience and potential fit with Etsy.</p>

		<p>I'm based in Europe and so am applying for this position as a remote role.</p>
		<h4 class='sub-heading'>Why Etsy ?</h4><span class='tagline'>Why I'm interested in Etsy</span>
		<p>I got into Software Development because I like to make things. I really relate alot to Etsy's core mission to enable others to make a living through making things. Outside of tech I knit and sow and have recently started making fun short movies.</p>
		<p>I'm looking for a position in a creative innovative company and I think Etsy would be a good fit.</p>
		<img class='profile' src='/profile-bright.jpg' alt='profile image'/>
		<h3 class='sub-heading under-image'>About Me</h3><span class='tagline'>A little bit about me </span>

		<p>My name is Niamh. I'm 24 and am currently in London. I'm originally from Dublin but have spent the last 12 months living as a digital nomad. I graduated from Trinity College Dublin in late 2015 with a degree in Computer Science and since then have been working remotely for VoxMarkets, a fintech startup based in London. </p>

		<p>Outside of my technical interests , I like to read, travel and watch alot of sports. I also run (badly) and love to ski. </p>

		<h4 class='sub-heading'>Why Consider Me ?</h4>
		<span class='tagline'>A short list of why I think I would be a suitable fit for Etsy.</span>
		<ul class='niamh-list'>
			<li>
				<img class='bullet' src='../assets/bullet_point.svg' alt="bullet for list">
				<span class='main-list-item'>Work Ethic</span>
				<span>I value a good work life balance but I also believe strongly in the idea that hard work is the best way to improve your situation in life. I am willing to work hard and put in long hours when necessary.  </span> </li>
			<li>
				<img class='bullet' src='../assets/bullet_point.svg' alt="bullet for list">
				<span class='main-list-item'> Willingness to learn </span>
				<span>I enjoy learning and always looking to improve technically and in other aspects of life. I think this is an important attribute in a developer. </span>
		 	</li>
			<li>
				<img class='bullet' src='../assets/bullet_point.svg' alt="bullet for list">
				<span class='main-list-item'>Technical Aptitude / Ability</span> 
				<span>I have gone into further detail on this <a href='#aptitude'>below</a> .</span> 
			</li>
		</ul>
	</div>
	<div class='experience'>
		<div class='experience-content'>
			<h4 class='sub-heading'>Experience</h4>
			<span class='tagline-inverted'>My revelant work and education history.</span>
			<div class='entry'>
				<div>
					<img class='bullet' src='../assets/bullet_point.svg' alt="bullet for list">
					<span class="title"> VoxMarkets</span> 
				</div>
				<div class='job-title'>Software Developer</div>
				<div class='date'>March 2016 - August 2017 </div>
				<div class='blurb'>I recently left my role at Vox, a startup based in London</div>
				<div class='blurb'> I worked with a range of technologies including Javascript, NodeJS and mySQL</div>
			</div>
			<div class='entry right'>
				<div>
					<span class="title"> Citi Innovation Lab</span> 
					<img class='bullet' src='../assets/bullet_point.svg' alt="bullet for list">
					
				</div>
				<div class='job-title'>Software Development Intern</div>
				<div class='date'>January 2015 - August 2015</div>
				<div class='blurb'>Worked on a number of prototype applications as well as an in production banking application </div>
			</div>
			<div class='entry'>
				<div>
					<img class='bullet' src='../assets/bullet_point.svg' alt="bullet for list">
					<span class="title"> Trinity College Dublin</span> 
				</div>
				<div class='job-title'>B.A(Mod) Computer Science</div>
				<div class='date'>September 2011 - September 2015</div>
			</div>
		</div>
	</div>
	<div class='application-layout-content'>
		<h4 id='aptitude' class='sub-heading'>Technical Aptitude</h4>
		<span class='tagline'>Some more of the technical stuff</span>
		<p>I believe I fulfil the technical requirements outlined in your job spec. I am happy to work hard to bridge any gap between my skillset and what is required with the relevant stack.</p>
		<p>Some of the technologies I have worked with in the past include</p>
		<p>React -- Javascript -- NodeJS -- MySql </p>

		<h4 class='sub-heading'>Links</h4>
		<span class='tagline'>Some more reading</span>
		<p class=''>My dynamic C.V is available  <a target="_blank" href='{{site.baseUrl}}/CV'>on this site.</a> (A lot of the content in this has been produced above).</p>
		<p class=''>I also have a more traditional PDF CV <a target="_blank" href='{{site.baseUrl}}/CV-august.pdf'>here</a></p> 
		<p><a target="_blank" href='{{site.baseUrl}}/projects'>Information on some of my personal projects</a></p>
		<p> Because you can never have too many links ....<a target="_blank" href='https://twitter.com/nileahtobhair'><img class='external-links' src='/pics/twitter2.svg' alt='link to twitter acc'/></a>
		<a target="_blank" href='https://twitter.com/nileahtobhair'><img class='external-links' src='/pics/github.svg' alt='link to github acc'/></a></p>
	</div>

	<h3 class='sub-heading end'>Thank you for taking the time to read my application.</h3>
	<p></p>
